...
(function() {
     ...
     */
    function nePublisher(container, initPublish, readyCallbackFun) {
        if (!nePublisher.__id) {
            nePublisher.__id = 100;
        }
        if (!nePublisher.__publishers) {
            nePublisher.__publishers = [];
        }
        nePublisher.__publishers.push(this);
        this.id = nePublisher.__id++;
     ...
     *      on_publisher_ready(cameras, microphones):int, when ne publisher ready, user can publish.
     *      on_publisher_error(code, desc):int, when ne publisher error, callback this method.
     *      on_publisher_warn(code, desc):int, when ne publisher warn, callback this method.
     */
    nePublisher.prototype.start = function () {
        var flashvars = {},
            params = {},
            attributes = {},
            self = this;;
        ...
        flashvars.on_publisher_ready = "__ne_on_publisher_ready";
        /**
         * [on_publisher_ready2 description] 测试用 用于接收as的返回值
         * @type {String}
         */
        flashvars.on_publisher_ready2 = "__ne_on_publisher_ready2";
        flashvars.on_publisher_error = "__ne_on_publisher_error";
        flashvars.on_publishInit_error = "__ne_on_publishInit_error";
        flashvars.on_publisher_warn = "__ne_on_publisher_warn";
        flashvars.on_publisher_getSDKVersion = "__ne_on_publisher_getSDKVersion"
        ...
        swfobject.embedSWF( //现在有个问题 此路径是相对于html页面的
            "../flash/release/nePublisher.swf?_version=" + ne_get_version_code(),// swf路径改成cdn路径
            this.container,
     ...
     */
    nePublisher.prototype.startPublish = function () {
        var i = 0;
     ...
     * @return {[type]} [description]
     */
    nePublisher.prototype.stopPublish = function () {
        var i = 0,
            publisher = null;
        for (i = 0; i < nePublisher.__publishers.length; i++) {
            publisher = nePublisher.__publishers[i];
            if (publisher.id != this.id) {
                continue;
            }
            nePublisher.__publishers.splice(i, 1);
            break;
        }
        this.callbackObj.ref.__stop();
    };
    nePublisher.prototype.startPreview = function (cameraIndex) {
        // var i = 0,
        //     publisher = null;
        // for (i = 0; i < nePublisher.__publishers.length; i++) {
        //     publisher = nePublisher.__publishers[i];
     ...
     */
    nePublisher.prototype.getDefinedErrors = function () {
        return this.errors;
    };
     ...
     */
    nePublisher.prototype.getCameraList = function () {
     ...
     */
    nePublisher.prototype.setCamera = function(cameraIndex) {
     ...
     */
    nePublisher.prototype.getMicroPhoneList = function () {
     ...
     */
    nePublisher.prototype.setMicroPhone = function (microPhoneIndex) {
     ...
     *                                                                   }
     */
    nePublisher.prototype.setDefination = function (definationObj) {
     ...
     */
    nePublisher.prototype.alterDefinedErrors = function (errorObject) {
     ...
     * @return {[type]}        [description]
     */
    nePublisher.prototype.resize = function (width, height) {
        this.width = width;
        this.height = height;
    };
    nePublisher.prototype.getSDKVersion = function () {
     ...
     * @return {[type]}                [description]
     */
    nePublisher.prototype.on_publisher_ready = function (cameraList, microPhoneList, readyCallbackFun) {
     ...
     */
    nePublisher.prototype.on_publisher_error = function (code, desc) {
        this.errorMessageCallbackFun(code, desc);
    };
    nePublisher.prototype.on_publishInit_error = function (code, desc) {
        this.readyCallbackFun(code, desc);
    };
    nePublisher.prototype.on_publisher_warn = function (code, desc) {
        this.errorMessageCallbackFun(code, desc);
    };

    function ne_get_version_code () {
        return "0.01"; 
    }
    ...
    window.nePublisher = nePublisher;

})();
function __ne_find_publisher (id) {
    var i,
        publisher;
    for (i = 0; i < nePublisher.__publishers.length; i++) {
        publisher = nePublisher.__publishers[i];
...
function __ne_on_publisher_ready (id, cameras, microphones) {
    var publisher = __ne_find_publisher(id);
    publisher.on_publisher_ready(cameras, microphones, publisher.readyCallbackFun);
}

function __ne_on_publisher_ready2 (video, audio) {
    // console.log(video, audio);
}

function __ne_on_publisher_error (id, code) {
    var publisher = __ne_find_publisher(id);
    publisher.code = code;
    publisher.on_publisher_error(code, publisher.errors["" + code]);
}
function __ne_on_publishInit_error (id, code) {
    var publisher = __ne_find_publisher(id);
    publisher.on_publishInit_error(code, publisher.errors["" + code])
    // publisher.code = code;
    // publisher.on_publisher_error(code, publisher.errors["" + code]);
}
function __ne_on_publisher_warn (id, code) {
    var publisher = __ne_find_publisher(id);
    publisher.code = code;
    publisher.on_publisher_warn(code, publisher.errors["" + code]);
}

function __ne_on_publisher_getSDKVersion (id, sdkVersion) {
    var publisher = __ne_find_publisher(id);
